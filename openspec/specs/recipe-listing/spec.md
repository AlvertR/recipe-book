...
The system SHALL render a `RecipeListPage` component that fetches all recipes using `useGetRecipesQuery()` and displays them in a responsive grid of cards.

#### Scenario: Recipes loaded successfully
- **WHEN** the API returns recipes successfully
- **THEN** the page SHALL display one `RecipeCard` per recipe in a grid layout (1 column on mobile, 2 columns on tablet, 3 columns on desktop)
